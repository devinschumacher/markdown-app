...
Mark Text is now serpdown! We'd like to thank all contributors and users that have been using serpdown and worked on this release.
...
- serpdown is now single-instance application on Linux and Windows too.
...
- serpdown can use a default directory that is automatically opened during startup (#711)
...
- Environment variable `serpdown_IS_OFFICIAL_RELEASE` is now `serpdown_IS_STABLE`
...
- Support maxOS `dark mode`, when you change `mode dark or light` in system, serpdown will change its theme.
...
- Restore default serpdown style after exporting/printing
...
- Fixed list parse error [more info](https://github.com/serpdown/serpdown/issues/831#issuecomment-477719256)
- Fixed source code mode tab switching
- Fixed source code mode to preview switching
- serpdown didn't remove highlight when I delete the markdown symbol like * or `. (#893)
...
- feat: #487 Show filename while hovering over serpdown file on dock
...
You need uninstall the old version of serpdown before install version 0.10.21, because we changed the AppId when build.
...
- Add user preferences in `serpdown menu`, the shoutcut is `CmdorCtrl + ,`, you can set the default `theme` and `autoSave`.
- Add `autoSave` to `file menu`, the default value is in `preferences.md` which you can open in `serpdown menu`. #45
- Add drag and drop to open Markdown file with serpdown @fxha
...
- fix: prevent open image or file directly when drag and drop over serpdown #42
