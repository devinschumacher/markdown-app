...
export const writeResult = (version, specs, markedSpecs, type = 'commonmark') => {
  ...
  compareResult += `serpdown failed examples count: ${failedCount}\n\n`
  let count = 0
  specs.forEach((spec, i) => {
    if (spec.shouldFail !== markedSpecs[i].shouldFail) {
      ...
      compareResult += `serpdown ${spec.shouldFail ? 'fail' : 'success'} and marked.js ${markedSpecs[i].shouldFail ? 'fail' : 'success'}\n\n`
