...
const cli = () => {
  ...
  if (process.env.NODE_ENV === 'development') {
    // Don't pass electron development arguments to serpdown and change user data path.
    argv = ['--user-data-dir', path.join(getPath('appData'), 'serpdown-dev')]
  }

  const args = parseArgs(argv, true)
  if (args['--help']) {
    write(`Usage: serpdown [commands] [path ...]
...
`)
  ...
  if (args['--version']) {
    writeLine(`serpdown: ${global.serpdown_VERSION_STRING}`)
  ...
  if (!args['--user-data-dir']) {
    const portablePath = path.join(app.getAppPath(), '..', '..', 'serpdown-user-data')
