...
if (!/^(darwin|win32|linux)$/i.test(process.platform)) {
  process.stdout.write(`Operating system "${process.platform}" is not supported! Please open an issue at "https://github.com/serpdown/serpdown".\n`)
  process.exit(1)
}
...
// Make serpdown a single instance application.
if (!process.mas && process.env.NODE_ENV !== 'development') {
  const gotSingleInstanceLock = app.requestSingleInstanceLock()
  if (!gotSingleInstanceLock) {
    process.stdout.write('Other serpdown instance detected: exiting...\n')
    app.exit()
  }
}

// serpdown environment is configured successfully. You can now access paths, use the logger etc.
...
try {
  ...
  const msgHint = err.message.includes('Config schema violation')
    ...
    : ''
  log.error(`Loading serpdown failed during initialization! ${msgHint}`, err)

  const EXIT_ON_ERROR = !!process.env.serpdown_EXIT_ON_ERROR
  const SHOW_ERROR_DIALOG = !process.env.serpdown_ERROR_INTERACTION
...
const serpdown = new App(accessor, args)
serpdown.init()
